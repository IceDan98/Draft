...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function navigateTo(pageName) { /* ... (Already modified) ... */
        ...
        if (pageName === 'home') { if(homePage) homePage.classList.remove('hidden'); if(currentAdminButton) currentAdminButton.classList.remove('hidden'); if(currentThemeButton) currentThemeButton.classList.remove('hidden'); if(currentLangButton) currentLangButton.classList.remove('hidden'); console.log("DEBUG navigateTo: Showing buttons for home page"); if (window.location.hash && currentUserRole !== 'admin') { currentUserRole = null; userTeamSide = null; history.pushState("", document.title, window.location.pathname + window.location.search); } updateUIText(currentLanguage); }
        else if (pageName === 'draft') { if(draftPage) draftPage.classList.remove('hidden'); console.log("DEBUG navigateTo: Hiding buttons for draft page"); if (!isDraftInitialized) { console.log("Initializing draft simulator for the first time..."); initializeAppDraft(); isDraftInitialized = true; } else { console.log("Draft already initialized, re-applying permissions for role:", currentUserRole); if (checkDraftElements()) { if (!currentUserRole) { currentUserRole = getRoleFromHash() || 'default'; } if (currentUserRole === 'team1') userTeamSide = 'blue'; else if (currentUserRole === 'team2') userTeamSide = 'red'; else userTeamSide = null; applyRolePermissions(currentUserRole); if(blueTeamNameH2) blueTeamNameH2.textContent = localStorage.getItem('lobbyTeam1Name') || translations[currentLanguage].blueTeamDefaultName; if(redTeamNameH2) redTeamNameH2.textContent = localStorage.getItem('lobbyTeam2Name') || translations[currentLanguage].redTeamDefaultName; updateUIText(currentLanguage); updateDraftUI(); } else { console.error("Draft elements not found when trying to re-apply permissions."); showStatusMessage("errorInitDraftElements", 5000); } } }
    }
    function getRoleFromHash() { /* ... (no changes needed) ... */ const hash = window.location.hash; if (hash.startsWith('#role=')) { const role = hash.substring(6); if (permissions[role] && role !== 'admin') { return role; } } return null; }

    // --- Home Page Logic ---
    function handleCreateLobby() { /* ... (Already modified) ... */
        console.log("handleCreateLobby called"); const team1Name = team1NameInput.value.trim() || translations[currentLanguage].blueTeamDefaultName; const team2Name = team2NameInput.value.trim() || translations[currentLanguage].redTeamDefaultName; localStorage.setItem('lobbyTeam1Name', team1Name); localStorage.setItem('lobbyTeam2Name', team2Name); const baseUrl = window.location.origin + window.location.pathname; const judgeLink = baseUrl + '#role=judge'; const team1Link = baseUrl + '#role=team1'; const team2Link = baseUrl + '#role=team2'; if (judgeLinkText) judgeLinkText.textContent = judgeLink; if (team1LinkText) team1LinkText.textContent = team1Link; if (team2LinkText) team2LinkText.textContent = team2Link; const openJudgeBtn = document.getElementById('openJudgeLinkButton'); const openTeam1Btn = document.getElementById('openTeam1LinkButton'); const openTeam2Btn = document.getElementById('openTeam2LinkButton'); if (openJudgeBtn) { openJudgeBtn.href = judgeLink; } else { console.warn("Judge 'Open' button not found"); } if (openTeam1Btn) { openTeam1Btn.href = team1Link; } else { console.warn("Team 1 'Open' button not found"); } if (openTeam2Btn) { openTeam2Btn.href = team2Link; } else { console.warn("Team 2 'Open' button not found"); } if (lobbyLinksDisplay) lobbyLinksDisplay.classList.remove('hidden'); showStatusMessage("lobbyCreatedMsg", 3000);
    }

    // --- Admin Button Logic ---
    function handleAdminClick() { /* ... (logic unchanged) ... */
        console.log("Admin button clicked."); currentUserRole = 'admin'; userTeamSide = null; const team1Name = team1NameInput.value.trim() || translations[currentLanguage].blueTeamDefaultName; const team2Name = team2NameInput.value.trim() || translations[currentLanguage].redTeamDefaultName; localStorage.setItem('lobbyTeam1Name', team1Name); localStorage.setItem('lobbyTeam2Name', team2Name); navigateTo('draft');
    }
    ...
    // --- Draft Logic Functions (Draft Specific - with permission checks added) ---
     ...
     function displayChampions() {
        ...
        championGridElement.appendChild(fragment); // Append the fragment containing cards
        filterChampions(); // Apply filters
    }
     // --- END FIXED displayChampions ---
    ...
    // --- Initial App Setup ---
     ...
     // --- ADDED: hashchange listener for navigation ---
     window.addEventListener('hashchange', () => {
         console.log("Hash changed:", window.location.hash);
         const roleFromHash = getRoleFromHash();
         if (roleFromHash && (!currentUserRole || roleFromHash !== currentUserRole )) {
             // Navigate to draft page only if the role is valid and different from current
             // Avoid re-navigating if already on the correct role page
             currentUserRole = roleFromHash;
             navigateTo('draft');
         } else if (!roleFromHash && currentPage !== 'home') {
             // If hash is removed or invalid, and we are not already home, go home
             navigateTo('home');
         }
     });
     // --- END ADDED hashchange listener ---
