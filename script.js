// script.js v6.9 - Refactor Role Filter Event Handling
document.addEventListener('DOMContentLoaded', () => {
    console.log("DOM Fully Loaded. Initializing App v6.9..."); // Version Updated
    ...
    let loadingIndicator, mainLayout, championGridElement, startButton, resetButton, undoButton, timerDisplay, championSearch, bluePicksContainer, redPicksContainer, blueColumn, redColumn, swapButton, clearPicksButton, toggleTimerButton, roleFilterButtonsContainer, /* Removed filterButtons NodeList */ confirmPickBanButton, priorityFilterButton, nextDraftButton, globallyBannedDisplay, globalBansBlueContainer, globalBansRedContainer, championTooltip, statusMessage, blueTeamNameH2, redTeamNameH2, blueScoreEl, redScoreEl, returnHomeButton;
    ...
    function checkDraftElements() {
        ...
        const elementsToCheck = [
            ...
            roleFilterButtonsContainer, // Check container
    ...
    async function initializeAppDraft() {
        ...
        try {
            ...
            // --- NEW: Event Delegation for Role Filters ---
            if (roleFilterButtonsContainer) {
                console.log("DEBUG: Attaching delegated click listener to #roleFilterButtons");
                roleFilterButtonsContainer.addEventListener('click', handleRoleFilterClick);
            } else {
                console.warn("Listener not attached: roleFilterButtonsContainer not found");
            }
    ...
    function applyRolePermissions(role) {
        ...
        if(roleFilterButtonsContainer) {
             // Disable individual buttons visually if needed (optional, delegation handles clicks)
             roleFilterButtonsContainer.querySelectorAll('.filter-button').forEach(btn => {
                 btn.disabled = !can('useRoleFilters');
             });
             // Optionally disable the container itself for visual cue, though clicks are handled
             // roleFilterButtonsContainer.style.pointerEvents = can('useRoleFilters') ? 'auto' : 'none';
        }
    ...
    function resetDraftFull(force = false) {
        ...
        if(championSearch) championSearch.value = ''; currentRoleFilter = 'All'; if(roleFilterButtonsContainer) { roleFilterButtonsContainer.querySelector('.filter-button.active')?.classList.remove('active'); roleFilterButtonsContainer.querySelector('.filter-button[data-role="All"]')?.classList.add('active'); } isPriorityFilterActive = false; if (priorityFilterButton) { priorityFilterButton.classList.remove('active'); priorityFilterButton.setAttribute('aria-pressed', 'false'); }
    ...
    // --- NEW: Role Filter Handler using Event Delegation ---
    function handleRoleFilterClick(event) {
        console.log("--- handleRoleFilterClick (Delegated) ---");
        const clickedButton = event.target.closest('.filter-button'); // Find the button that was clicked

        if (!clickedButton) {
            console.log("Click was not on a filter button.");
            return; // Exit if the click wasn't on a button inside the container
        }
        ...
        // Update active class on buttons within the container
        if (roleFilterButtonsContainer) {
            roleFilterButtonsContainer.querySelectorAll('.filter-button').forEach(btn => {
            ...
            console.warn("roleFilterButtonsContainer not found for updating active class.");
        }
