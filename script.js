// script.js v7.20 - Rewritten Clear Button logic (resetCurrentGamePicksBans)
document.addEventListener('DOMContentLoaded', () => {
    console.log("DOM Fully Loaded. Initializing App v7.20..."); // Version Updated
    ...
    const translations = { /* ... (translations as before) ... */
        ru: {
            ...
            clearPicksTitle: "Очистить пики/баны текущей игры", // Updated title slightly
            ...
            resetCurrentConfirmation: "Остановить текущий драфт и очистить пики/баны этой игры?", // Simplified confirmation text
            resetCurrentGameKeptInfo: "Пики/баны текущей игры очищены (ники/глоб. баны сохранены).",
            resetCurrentComplete: "Пики/баны текущей игры{global} очищены.", // Fallback
            resetCurrentCompleteKeptGlobal: "Текущая игра очищена.", // Fallback if keepGlobal was used differently
        ...
        en: {
            ...
            clearPicksTitle: "Clear picks/bans for the current game", // Updated title slightly
            ...
            resetCurrentConfirmation: "Stop the current draft and clear this game's picks/bans?", // Simplified confirmation text
            resetCurrentGameKeptInfo: "Current game picks/bans cleared (nicknames/global bans kept).", // New message
            resetCurrentComplete: "Current game's picks/bans{global} cleared.", // Fallback
            resetCurrentCompleteKeptGlobal: "Current game cleared.", // Fallback
    ...
    function restoreSlotPlaceholder(slotElement, slotId, nicknameText = '') {
        // --- DEBUG LOG ---
        console.log(`DEBUG restoreSlotPlaceholder: slotId=${slotId}, nicknameText="${nicknameText}"`);
        ...
        if (slotId && slotId.includes('-pick-')) {
            addNicknameInput(slotElement, nicknameText); // Re-adds the input DIV with the passed text
            // Ensure state reflects the passed text (might be redundant but safe)
            let nicknames = getLobbyItem('pickNicknames', {});
            nicknames[slotId] = nicknameText;
            setLobbyItem('pickNicknames', nicknames);
            console.log(`DEBUG restoreSlotPlaceholder: Added nickname input for ${slotId} with text "${nicknameText}"`);
        } else {
            ...
            if (nicknames.hasOwnProperty(slotId)) {
                 ...
                 setLobbyItem('pickNicknames', nicknames);
                 console.log(`DEBUG restoreSlotPlaceholder: Removed nickname state for ban slot ${slotId}`);
            }
        }
        // --- DEBUG LOG ---
        const finalNickDiv = slotElement.querySelector('.nickname-input');
        console.log(`DEBUG restoreSlotPlaceholder: Final innerHTML for ${slotId}:`, slotElement.innerHTML, ` Nickname div text: ${finalNickDiv ? finalNickDiv.textContent : 'N/A'}`);
    ...
    function resetDraftFull(force = false) {
        ...
        setLobbyItem('currentRoleFilter', 'All'); // Reset filter state
        setLobbyItem('isPriorityFilterActive', false); // Reset filter state
        if(filterButtons) { filterButtons.forEach(btn => btn.classList.remove('active')); filterButtons[0]?.classList.add('active'); } if (newPriorityFilterButton) { newPriorityFilterButton.setAttribute('aria-pressed', 'false'); }
    ...
    // --- REWRITTEN: resetCurrentGamePicksBans (Clear Button Logic) ---
    function resetCurrentGamePicksBans(force = false, keepGlobal = true) { // keepGlobal defaults to true now
         console.log("--- resetCurrentGamePicksBans (v7.20) ---", { force, keepGlobal });
         ...
         // Confirmation dialog if draft is in progress
         if (!force && isStarted && !isComplete) {
             const confirmationMessage = translations[currentLanguage].resetCurrentConfirmation || "Stop the current draft and clear this game's picks/bans?"; // Fallback text
         ...
         // 2. Preserve Nicknames (Read Before Clearing Other State)
         const currentNicknames = getLobbyItem('pickNicknames', {});
         console.log("Nicknames read before clearing slots:", currentNicknames);

         // 3. Reset Core State (Keep Nicknames, Keep Globals if keepGlobal=true)
         setLobbyItem('currentStep', 0);
         setLobbyItem('draftHistory', []);
         // Nicknames state ('pickNicknames') is NOT cleared
         setLobbyItem('isDraftComplete', false);
         setLobbyItem('isDraftStarted', false); // Stop the draft logically
         setLobbyItem('previewedChampionId', null);
         setLobbyItem('previewedSlotId', null);
         setLobbyItem('timerSeconds', getLobbyItem('draftTimerDuration', 30)); // Reset timer value

         let currentSelected = new Set();
         if (keepGlobal) {
             const currentGlobalDisabled = new Set(getLobbyItem('globallyDisabledChampions', []));
             currentGlobalDisabled.forEach(id => currentSelected.add(id));
             console.log("Keeping global bans.");
             // Keep globalBanHistory - do not clear it
         } else {
              // This part should not run for the Clear button now, but kept for potential future use
              setLobbyItem('globallyDisabledChampions', []);
              setLobbyItem('globalBanHistory', []);
              console.log("Global bans cleared for lobby.");
         }
         setLobbyItem('selectedChampions', Array.from(currentSelected)); // Save updated set (only globals)

         // 4. Reset UI Slots (Directly manipulate, preserve nickname divs)
         ...
         document.querySelectorAll('.pick-slot, .ban-slot').forEach((slot) => {
             ...
             slot.style.backgroundImage = ''; // Remove background image if any

             // Remove only the image if it exists
             const img = slot.querySelector('img');
             if (img) {
                 console.log(`DEBUG Clear: Removing image from ${slot.id}`);
                 img.remove();
             }

             // For pick slots, find/update the nickname div
             if (slot.id.includes('-pick-')) {
                 let nicknameDiv = slot.querySelector('.nickname-input');
                 const expectedNick = currentNicknames[slot.id] || ''; // Get preserved nickname
                 console.log(`DEBUG Clear: Processing pick slot ${slot.id}. Expected nick: "${expectedNick}". Found div:`, !!nicknameDiv);

                 if (!nicknameDiv) {
                     // If div doesn't exist (shouldn't happen often), recreate it
                     addNicknameInput(slot, expectedNick); // addNicknameInput also sets state, which is fine
                     console.log(`DEBUG Clear: Re-added nickname input for ${slot.id} with text "${expectedNick}"`);
                     nicknameDiv = slot.querySelector('.nickname-input'); // Re-select if just added
                 } else {
                     // If div exists, just update its text content
                     nicknameDiv.textContent = expectedNick;
                     console.log(`DEBUG Clear: Updated existing nickname input for ${slot.id} to text "${expectedNick}"`);
                 }
                 // Ensure placeholder attribute is set for styling ::before
                 if(nicknameDiv) {
                     const placeholderText = translations[currentLanguage]?.pickSlotNicknamePlaceholder || 'Player';
                     nicknameDiv.dataset.placeholder = placeholderText;
                 }

             } else {
                  // For ban slots, ensure no nickname div exists and clear other content
                  const nicknameDiv = slot.querySelector('.nickname-input');
                  if (nicknameDiv) {
                      console.log(`DEBUG Clear: Removing nickname div from ban slot ${slot.id}`);
                      nicknameDiv.remove();
                  }
                 // Clear potential error messages etc. from ban slots
                 slot.innerHTML = '';
             }
             // Reset aria-label
             const baseAriaLabel = slot.getAttribute('aria-label')?.split(':')[0] || (slot.id.includes('-pick-') ? `Pick ${slot.id.slice(-1)}` : `Ban ${slot.id.slice(-1)}`);
             slot.setAttribute('aria-label', `${baseAriaLabel}: Empty`);
         });
         console.log("Finished slot clearing loop.");

         // 5. Update Other UI Elements
         resetTimerDisplay(); // Update timer display based on reset state
         displayGloballyBanned(); // Update global ban display (should show kept bans)
         updateChampionAvailability(); // Update champion grid availability
         filterChampions(); // Re-apply filters
         updateDraftUI(); // Update overall UI state (buttons etc.)

         showStatusMessage("resetCurrentGameKeptInfo", 2500); // Use the specific message
     }
    // --- END REWRITTEN resetCurrentGamePicksBans ---


    // --- Other Handlers (Refactored for Lobby State) ---
    function handleStartDraft() { /* ... (Already modified) ... */
    ...
    function filterChampions(roleFilterValue = null) {
        ...
        const roleFilter = roleFilterValue !== null ? roleFilterValue : getLobbyItem('currentRoleFilter', 'All'); // Use argument or state
        const priorityFilterActive = getLobbyItem('isPriorityFilterActive', false); const lobbySelectedChamps = new Set(getLobbyItem('selectedChampions', [])); const lobbyGloballyDisabled = new Set(getLobbyItem('globallyDisabledChampions', [])); const combinedDisabled = new Set([...lobbySelectedChamps, ...lobbyGloballyDisabled]);
